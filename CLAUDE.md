...
- **PR 올릴 때 `PLAN.md` / `README.md` 동기화** — 변경 사항이 다음에 해당하면 같은 PR에 문서 수정도 포함한다:
  - `PLAN.md`: Week N 체크리스트 항목 완료/추가/변경, 마일스톤 일정 조정, 리스크 대응 변경
  - `README.md`: 디렉토리 구조 변경, 실행 방법 변경, 기술 스택/의존성 추가, 기능 설명에 영향 주는 변경
  - 코드 변경과 별개 커밋으로 분리하되 동일 PR에 포함 (예: `docs: PLAN.md Week 1 체크박스 갱신`)
  - 별도 PR로 미루지 말 것 — 머지 시점에 main의 코드와 문서 상태가 항상 일치해야 함
